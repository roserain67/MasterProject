"""
离散动作 TQC（Truncated Quantile Critics）：分位数 Q 网络，与 SAC/TD3 同环境、同数据，结果保存到指定 log_dir。
"""
import os
...
class QuantileCritic(nn.Module):
    """输出每个动作的 N 个分位数值 -> (batch, n_actions, n_quantiles)"""
    def __init__(self, state_dim, n_actions, n_quantiles=N_QUANTILES, hidden=256):
        super().__init__()
...
def quantile_huber_loss(quantiles, target, kappa=QUANTILE_HUBER_KAPPA):
...
def train(encoder_model=None, train_sequences=None, test_by_unit=None, log_dir=None, num_episodes=300):
    ...
    if log_dir is None:
        log_dir = os.path.join(os.path.dirname(__file__), "..", "TQC-logs", "1")
    ...
    critic = QuantileCritic(state_dim, n_actions).to(DEVICE)
    critic_target = QuantileCritic(state_dim, n_actions).to(DEVICE)
    ...
    for ep in range(1, num_episodes + 1):
        ...
        for t in range(MAX_LEN):
            ...
            if len(replay) >= BATCH:
                ...
                with torch.no_grad():
                    ...
                    q_next_mean = q_quant_next.mean(dim=-1)
                    q_next = q_next_mean.max(1)[0]
                ...
                loss_c = quantile_huber_loss(q_a_quant, td_target)
        ...
        if ep % 10 == 0:
            print(f"[TQC] Ep {ep}: reward={episode_reward:.2f}  entropy={mean_entropy:.4f}")
